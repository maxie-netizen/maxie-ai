/* If You Copy, Don't Delete This Credit!!! 
  Don't Sell This Script Or I Will Take Immediate Action 
  Those Who Sell This Script - Report/Ban Their Account Or Just Beat Them Up
  Move To Pairing Code
  For Those Who Don't See The Code, It's Caused By Too Many Plugins
  If You Want To Take The Session, Backup All Plugin Files & Delete All Plugin Files
  After That You Can Restore All Plugin Files To Be Used Again
...
if(usePairingCode && !conn.authState.creds.registered) {
		...
		let phoneNumber = ''
		do {
			phoneNumber = await question(chalk.blueBright('Input a Valid number start with region code. Example : 254xxx:\n'))
...
async function connectionUpdate(update) {
    ...
    if (isOnline == true) {
        ...
        console.log(chalk.red('Inactive Status'));
    }
...
global.reloadHandler = async function (restatConn) {
    ...
    try {
	// If you're using replit, use sevenHoursLater and add // to const Handler
	// Default: server/vps/panel, replit + 7 hours for Indonesia time
  ...
  conn.sRestrictOn = 'Edit Group Info changed to admins only!'
...
async function _quickTest() {
    ...
    if (s.ffmpeg && !s.ffmpegWebp) {
        conn.logger.warn('Stickers may not be animated without libwebp in ffmpeg (--enable-ibwebp while compiling ffmpeg)')
    }

    if (!s.convert && !s.magick && !s.gm) {
        conn.logger.warn('Sticker features may not work without imagemagick and libwebp in ffmpeg not installed (pkg install imagemagick)')
    }
...
setInterval(async () => {
  ...
  return [d, ' Days ', h, ' Hours ', m, ' Minutes ', s, ' Seconds '].map(v => v.toString().padStart(2, 0)).join('')}
